...
if __name__ == '__main__':
    ...
    mu = 0
    sigma = 1
    sample_slice = -1 + 2* np.random.random((5, 64, 20))
    ...
    x_train_random = np.array([sample_slice + np.random.normal(mu, sigma, (5, 64, 20)) for i in range(1800)])
    x_val_random = np.array([sample_slice + np.random.normal(mu, sigma, (5, 64, 20)) for i in range(900)])
    # 归一化
    x_train_random = Data_MinMax_Scaler(x_train_random)
    x_val_random = Data_MinMax_Scaler(x_val_random)
    train_cnn_AE_2((5, 64, 20), x_train_random, x_train_random, x_val_random, x_val_random, epochs=500)
